  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <motion.button
        ...
        className="w-full sm:w-40 md:w-48 group relative flex items-center gap-2 px-2 sm:px-3 md:px-4 py-2 sm:py-2.5 bg-[#1e1e2e]/80 hover:bg-[#262637] 
      ...
      >
        ...
        <Palette className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-gray-400 group-hover:text-gray-300 transition-colors flex-shrink-0" />

        <span className="text-gray-300 text-xs sm:text-sm min-w-0 flex-1 text-left group-hover:text-white transition-colors truncate">
          {currentTheme?.label}
        </span>

        {/* color indicator */}
        <div className="relative w-3 h-3 sm:w-4 sm:h-4 rounded-full border border-gray-600 group-hover:border-gray-500 transition-colors flex-shrink-0"
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute top-full left-0 mt-2 w-full sm:w-full sm:min-w-60 bg-[#1e1e2e]/95 
            backdrop-blur-xl rounded-xl border border-[#313244] shadow-2xl py-2 z-50 max-h-[70vh] overflow-y-auto"
          >
